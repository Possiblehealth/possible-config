...
FROM (SELECT
      ...
      FROM
        (SELECT
         ...
         WHERE rag.report_group_name = 'All Ages' AND cn.name IN ('ANC-USG placenta position','ECG note') AND
        ...
        (SELECT
         ...
         FROM (SELECT
               ...
               WHERE
                 cn.name IN ('ANC-USG placenta position','ECG note') AND cast(v.date_stopped AS DATE) BETWEEN '#startDate#' AND '#endDate#' AND
                 obs.voided IS FALSE
